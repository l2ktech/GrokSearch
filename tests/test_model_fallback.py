...
async def test_provider_falls_back_when_grok_420_returns_502(monkeypatch):
    ...
    monkeypatch.setattr(config, "get_fallback_models_for", lambda model: ("grok-4.20-fast",))
    ...
    result = await provider._execute_stream_with_retry(
        ...
        payload={"model": "grok-4.20-0309", "messages": [], "stream": False},
    )

    assert result == "fallback-ok"
    assert attempted_models == ["grok-4.20-0309", "grok-4.20-fast"]
